// Lokasi: netlify/functions/telegram-webhook-d2.js
const admin = require('firebase-admin');
const axios = require('axios');

// --- KONFIGURASI KHUSUS UNTUK DEVICE 2 ---
const DEVICE_ID = 'device2';
const BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN_DEVICE2;
// -----------------------------------------
...
async function sendTelegramReply(chatId, text) {
  ...
  try {
    ...
    console.error("Error sending D2 reply:", error.message);
  }
}
...
async function isDeviceOnline() {
  try {
    ...
    console.error(`Error checking D2 status:`, error);
    return false;
  }
}
...
exports.handler = async function(event) {
  ...
  try {
    ...
    return { statusCode: 200, body: 'D2 command processed.' };
  } catch (error) {
    console.error("Webhook D2 Error:", error);
